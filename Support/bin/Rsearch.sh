...
HEAD="$BPATH"/generated/head.html
DATA="$BPATH"/generated/data.html
...
if [ "$AS" == "1" ]; then
	...
	# 
	FILE=$(egrep -i "^($REFS$TERM)	" "$BPATH"/generated/help.index | awk '{print $2;}' | perl -pe 's!/latex/!/html/!;s!tex$!html!' | sort | uniq)
	...
	FILE=$(egrep -m 260 -l -i -r "$TERM" "${R_HOME:-/Library/Frameworks/R.framework/Resources}"/library/*/help/ | perl -pe  's!/help/(.*)!/html/$1.html!;s/AnIndex\.html/00Index.html/')
